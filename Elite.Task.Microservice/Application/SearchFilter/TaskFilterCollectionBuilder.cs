    ...
    public class TaskFilterCollectionBuilder<T> where T : EliteTask, new()
    {
        ...
        public Expression<Func<T, bool>> BuildFilter()
        {
            ...
            if (_keywords.ResponsibleDivision?.Count > 0)
            {
                ...
                {
                    responsibleDivFilter.AddSearchCriteria(StringSearchOperator.Equals);
                    responsibleDivFilter.SearchOprators.ForEach(op =>
                    {

                        if (op == StringSearchOperator.Contains)
                        {
                            responsibleDivFilter.Predicate = responsibleDivFilter.Predicate.Or(p => p.ResponsibleDivision != null && p.ResponsibleDivision.ToString().ToUpper().Contains(x.ToUpper()));
                        }
                        if (op == StringSearchOperator.Equals)
                        {
                            responsibleDivFilter.Predicate = responsibleDivFilter.Predicate.Or(p => p.ResponsibleDivision != null && p.ResponsibleDivision.ToString().ToUpper().Equals(x.ToUpper()));
                        }

                        if (op == StringSearchOperator.StartsWith)
                        {
                            responsibleDivFilter.Predicate = responsibleDivFilter.Predicate.Or(p => p.ResponsibleDivision != null && p.ResponsibleDivision.ToString().ToUpper().StartsWith(x.ToUpper()));
                        }

                        if (op == StringSearchOperator.EndsWith)
                        {
                            responsibleDivFilter.Predicate = responsibleDivFilter.Predicate.Or(p => p.ResponsibleDivision != null && p.ResponsibleDivision.ToString().ToUpper().EndsWith(x.ToUpper()));
                        }
                    });
                    // _orderByExpression.Add(new OrderByExpression<T, string>(u => u.ResponsibleJson));
            ...
            #endregion
            #region  task  co responsible Division
            if (_keywords.CoResponsiblesDivision?.Count > 0)
            {
                var coresponsibleDivFilter = new StringTypeFilter<T>();


                _keywords.CoResponsiblesDivision.ForEach(x =>
                {
                    coresponsibleDivFilter.AddSearchCriteria(StringSearchOperator.Contains);
                    coresponsibleDivFilter.SearchOprators.ForEach(op =>
                    {

                            coresponsibleDivFilter.Predicate = coresponsibleDivFilter.Predicate.Or(p => p.CoResponsibleDivisions != null && p.CoResponsibleDivisions.ToUpper().Split(';', StringSplitOptions.None).Contains(""));
           
                    });
                    // _orderByExpression.Add(new OrderByExpression<T, string>(u => u.ResponsibleJson));
                    _filters.Add(coresponsibleDivFilter);
                });
            }
            #endregion
            ...
            #endregion
            #region task Meeting data
            if (_keywords.MeetingStartDate.HasValue && _keywords.MeetingEndDate.HasValue)
            {
                var dueDateFilter = new DateTypeFilter<T>();
                dueDateFilter.AddSearchCriteria(DateSearchOperator.Between);
                dueDateFilter.SearchOprators.ForEach(op =>
                {
                    ...
                    if (op == DateSearchOperator.Between)
                    {
                        dueDateFilter.Predicate = dueDateFilter.Predicate.Or(p => (p.MeetingDate.HasValue) ? p.MeetingDate.Value >= _keywords.MeetingStartDate.Value && p.MeetingDate.Value <= _keywords.MeetingEndDate.Value : false);
                    }

                    if (op == DateSearchOperator.CurrentDate)
                    {
                        dueDateFilter.Predicate = dueDateFilter.Predicate.Or(p => p.MeetingDate.Value >= _keywords.MeetingStartDate.Value && p.MeetingDate.Value <= DateTime.Now);
                    }
