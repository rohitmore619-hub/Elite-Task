...
namespace Elite_Task.Microservice.Application.CQRS.Commands
{
    public class TaskCommandHandler : BaseCommandHandler<TaskCommand>, IRequestHandler<TaskCommand, long>
    {
        ...
        private TaskPersonCommand _taskOwner;
        ...
        private async Task<long> SaveTaskAndAttachment(TaskCommand request)
        {
            ...
            if (task != null)
            {
                ...
                _taskOwner = request.TaskOwner==null? _createdBy : request.TaskOwner;
                task.TaskOwner = JsonConvert.SerializeObject(_taskOwner);
        ...
        private async Task Update(TaskCommand request, EliteTask task, EliteTask parentTask = null, bool isSubTask = false)
        {
            ...
            request.FollowUpRequired = task.FollowUpRequired.HasValue ? task.FollowUpRequired.Value : false;
        ...
        private EliteTask CreateMapper(EliteTask task, TaskCommand request, EliteTask parentTask = null)
        {
            ...
            task.CoResponsibleDivisions = request.CoResponsibleDivisions;
            task.FollowUpRequired = request.FollowUpRequired;
            ...
            if (request.Id > 0)
            {
                ...
                _taskOwner = task.TaskOwner != null ? JsonConvert.DeserializeObject<TaskPersonCommand>(task.TaskOwner) : JsonConvert.DeserializeObject<TaskPersonCommand>(task.CreatedBy);
                task.TaskOwner = JsonConvert.SerializeObject(_taskOwner);
        ...
        private async Task<MeetingTaskEvent> CreateTaskMapperAsync(TaskCommand command, long taskId)
        {
            ...
            if (subtaskEntity != null)
            {
                subtaskEntity.ForEach(x => subtask.Add(new MeetingMinuteTaskEvent()
                {
                    ...
                    TaskOwner = x.TaskOwner != null ? JsonConvert.DeserializeObject<TaskPersonCommand>(x.TaskOwner) : JsonConvert.DeserializeObject<TaskPersonCommand>(x.CreatedBy),
                    SubTaskCount = 0,
                    TaskId = x.Id,
                    Title = x.Title,
                    Status = x.Status,
        ...
        private void postForCompleteCommitteManagersNotification(EliteTask task, TaskCommand request)
        {
            ...
            this._taskNotifications.Add(new EmailNotificationEvent
            {
                ...
                {
                    CreatedBy =request.TaskOwner!=null?request.TaskOwner: _createdBy,
                    Responsible = request.TaskOwner != null ? request.TaskOwner : _createdBy,
                        ...
                        foreach (var user in item.users)
                        {
                            ...
                            if (!(request.notifyUsers == (long)Notify.NOTIFY_ALL && (coresponsible.Uid.ToUpper() == request.Responsible.Uid.ToUpper())))
                                AddTaskEmailNotification(task, NotificationActionType.ASSIGN_TO_CO_RESPONSIBLE, _createdBy,
                                    ...
                                    userRole, null, request.IsDemoUser, null, committeePoolIdEmail, null, null, committeePoolIdName, closureComment: request.ClosureComment);
                        }
                    }
                    else
                    {
                        ...
                        if (!(request.notifyUsers == (long)Notify.NOTIFY_ALL && (coresponsible.Uid.ToUpper() == request.Responsible.Uid.ToUpper())))
                            AddTaskEmailNotification(task, NotificationActionType.ASSIGN_TO_CO_RESPONSIBLE, _createdBy, coresponsible,
                                ...
                                null, committeePoolIdEmail, null, null, committeePoolIdName, closureComment: request.ClosureComment);
                    }
                }
        }
        ...
        private void postForUpdateResponsibleNotification(EliteTask task, TaskCommand request)
        {
            ...
            // Check if initial participants ever received task assignment emails
            bool initialEmailsSent = HasInitialEmailsBeenSent(task);
            ...
            if ((flagUpdated == false && statusUpdate == false) || (flagUpdated == false && statusUpdate == true && (existingResponsible.Uid.ToUpper() != responsible.Uid.ToUpper())))
            {
                ...
                AddTaskEmailNotification(task, actionType, _createdBy, responsible, request.Title, task.Id,
                    ...
                    committeePoolIdEmail, null, null, committeePoolIdName, closureComment: request.ClosureComment);

                if (existingResponsible.Uid.ToUpper() != responsible.Uid.ToUpper() && initialEmailsSent)
                {
                    userRole = GetUserRole(_modifiedBy != null ? _modifiedBy.Uid : _createdBy.Uid, task.CommitteeId);
                    // Only send "no longer responsible" email if initial emails were sent
                    AddTaskEmailNotification(task, NotificationActionType.NOTIFY_OLD_RESPONSIBLE, _createdBy, existingResponsible,
                        ...
                        request.IsDemoUser, null, committeePoolIdEmail, null, null, committeePoolIdName, closureComment: request.ClosureComment);
                }
            }
        ...
        private void postForUpdateCoResponsibleNotification(EliteTask task, TaskCommand request)
        {
            ...
            if (existingCoResponsiblelist != null)
            ...
            List<TaskPersonCommand> responsibletoAddDelta = null;


            if (existingCoResponsibleUsersList != null)
            {
                if (currentListUsers != null)
                {

                    if (currentListUsers.Where(x => !existingCoResponsibleUsersList.Any(y => y.Uid == x.Uid)).ToList().Count != 0 || (request.MeetingStatus == 1 && request.Status == 1))
                    {
                        responsibletoAddDelta = currentListUsers.Where(x => existingCoResponsibleUsersList.Any(y => y.Uid == x.Uid)).ToList();
                    }
                    if (existingCoResponsibleUsersList.Where(x => !currentListUsers.Any(y => y.Uid == x.Uid)).ToList().Count != 0 || (request.MeetingStatus == 1 && request.Status == 1))
                    {
                        responsibletoRemove = existingCoResponsibleUsersList.Where(x => !currentListUsers.Any(y => y.Uid == x.Uid)).ToList();
                    }

                    //send only to the current list of co responsibles in case of no delta
                    responsibletoAdd = currentListUsers.ToList();
            ...
            //if notify flags have not updated deletion mails will trigger as we take care of delta
            if ((flagUpdated == false && statusUpdate == false) || (flagUpdated == false && statusUpdate == true && (responsibletoAddDelta != null || responsibletoRemove != null)))
            {
                //removing old co-responsibles
                if (responsibletoRemove != null && initialEmailsSent)
                    foreach (var item in responsibletoRemove)
                    {
                        var coresponsible = new TaskPersonCommand(item.Uid, item.DisplayName);

                        //removedCoResponsible.Add(removeAttende);
                        // Only send "no longer co-responsible" email if initial emails were sent
                ...
                //adding new co-responsibles
                if (responsibletoAddDelta != null)
                    foreach (var item in responsibletoAddDelta)
                    {
                        ...
                        if (!(request.notifyUsers == (long)Notify.NOTIFY_ALL && (coresponsible.Uid.ToUpper() == request.Responsible.Uid.ToUpper())))
                            AddTaskEmailNotification(task, actionType, _createdBy, coresponsible, request.Title, task.Id,
                                ...
                                request.IsDemoUser, meetingInfo, committeePoolIdEmail, null, null, committeePoolIdName, closureComment: request.ClosureComment);
                    }
            }
            if (statusUpdate == true)
            {
                            ...
                            foreach (var user in item.users)
                            {
                                ...
                                if (!(request.notifyUsers == (long)Notify.NOTIFY_ALL && (coresponsible.Uid.ToUpper() == request.Responsible.Uid.ToUpper())))
                                    AddTaskEmailNotification(task, actionType, _createdBy, user, request.Title, task.Id, description,
                                        ...
                                        meetingInfo, committeePoolIdEmail, null, null, committeePoolIdName, closureComment: request.ClosureComment);
                            }
                        }
                        else
                        {
                        ...
                        if (responsible.users != null)
                        {
                            foreach (var user in responsible.users)
                            {
                                ...
                                AddTaskEmailNotification(task, actionType, _createdBy, user, request.Title, task.Id,
                                        description, task.DueDate, request.CommitteeId, committeeName, userRole, null, request.IsDemoUser, meetingInfo,
                                        committeePoolIdEmail, _modifiedBy, 15, committeePoolIdName, closureComment: request.ClosureComment);
                            }
                        }
                        else
                        {
                    ...
                    foreach (var responsible in coResponsibleList)
                    {
                        ...
                        if (responsible.users != null)
                        {
                            foreach (var user in responsible.users)
                            {
                                ...
                                AddTaskEmailNotification(task, actionType, _createdBy, user, request.Title, task.Id,
                                    description, task.DueDate, request.CommitteeId, committeeName, userRole, null, request.IsDemoUser, meetingInfo,
                                    committeePoolIdEmail, _modifiedBy, 15, committeePoolIdName, closureComment: request.ClosureComment, rejectionComment: request.RejectionComment);
                            }
                        }
                        else
                        {
        ...
        private void statusMailResponsible(EliteTask task, TaskCommand request)
        {
            ...
            if (task.Status.Equals((int)CommonLib.TaskStatus.Assigned) && request.Status.Equals((int)CommonLib.TaskStatus.Completed))
            {
                ...
                AddTaskEmailNotification(task, actionType, _createdBy, responsible, request.Title,
                    ...
                    meetingInfo, committeePoolIdEmail, _modifiedBy, 15, committeePoolIdName, closureComment: request.ClosureComment);
                return;
            }
            else if (task.Status.Equals((int)CommonLib.TaskStatus.InProgress) && request.Status.Equals((int)CommonLib.TaskStatus.Completed))
            {
            ...
            else if (task.Status.Equals((int)CommonLib.TaskStatus.Completed) && request.Status.Equals((int)CommonLib.TaskStatus.InProgress))
            {
                ...
                AddTaskEmailNotification(task, actionType, _createdBy, responsible, request.Title,
                    task.Id, description, task.DueDate, request.CommitteeId, committeeName, userRole, null, request.IsDemoUser,
                    meetingInfo, committeePoolIdEmail, _modifiedBy, 15, committeePoolIdName, closureComment: request.ClosureComment, rejectionComment: request.RejectionComment);
                return;
            }
        }
        ...
        private void postForCreateSubTaskResponsibleNotification(EliteTask task, TaskCommand request)
        {
            ...
            AddTaskEmailNotification(task, NotificationActionType.ASSIGN_TO_RESPONSIBLE, _createdBy,
                ...
                userRole, null, request.IsDemoUser, meetingInfo, committeePoolIdEmail, null, null, committeePoolIdName, closureComment: request.ClosureComment);
        }
            ...
            if ((statusUpdate == false || (statusUpdate == true && (existingResponsible.Uid.ToUpper() != responsible.Uid.ToUpper()))) && task.Status != 3)
            {
                ...
                if (existingResponsible.Uid.ToUpper() != responsible.Uid.ToUpper())
                {
                    ...
                    AddTaskEmailNotification(task, NotificationActionType.NOTIFY_OLD_RESPONSIBLE, _createdBy,
                        ...
                        userRole, responsible.DisplayName, request.IsDemoUser, null, committeePoolIdEmail, null, null, committeePoolIdName, closureComment: request.ClosureComment);
                }
            }
        ...
        private bool HasInitialEmailsBeenSent(EliteTask task)
        {
            // If task was created from Meeting Agenda section (has MeetingId and AgendaId), 
            // initial emails were NOT sent to Normal Responsible/Co-Responsible
            if (task.MeetingId.HasValue && task.MeetingId.Value > 0 && 
                task.AgendaId.HasValue && task.AgendaId.Value > 0)
            {
                return false;
            }
            
            // For tasks created directly from Task List section (no MeetingId/AgendaId), 
            // initial emails were sent
            return true;
        }
        ...
        private void AddTaskEmailNotification(EliteTask task, NotificationActionType actionType, TaskPersonCommand createdBy, TaskPersonCommand responsible,
            ...
            MeetingInfo meetingInfo, string poolIdEmailId, TaskPersonCommand modifiedBy, int? status, string committeePoolIdName, string closureComment = "", string rejectionComment = "")
        {
            ...
            if (responsible.Uid.Length < 32)
            {
                this._taskNotifications.Add(new EmailNotificationEvent
                {
                    ...
                    Message = new TaskEvent
                    {
                        CreatedBy = task.TaskOwner!=null? JsonConvert.DeserializeObject<TaskPersonCommand>(task.TaskOwner) : createdBy,
