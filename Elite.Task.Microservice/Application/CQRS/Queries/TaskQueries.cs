...
namespace Elite_Task.Microservice.Application.CQRS.Queries
{
    public class TaskQueries : BaseDataAccess, ITaskQueries
    {
        ...
        public async Task<IList<QueriesTaskListDto>> GetTaskByPagination(int? page, int listType, TaskSearchKeywords searchKeywords, HttpResponse response, FilterActionEnum topicFilterAction)
        {
            try
            {
                ITaskRolesAndRightFilterBuilder<EliteTask> rolesBuilder = TaskRolesAndRightsFilterFactory.GetTaskRolesAndRightsFilterObject(topicFilterAction, this.rolesPermissions.UserRolesAndRights, this._requestContext);
        ...
        {
              var tasks = await this.GetDBSet<EliteTask>()
             .Where(p =>
             (p.Status == (int)CommonLib.TaskStatus.Assigned ||
             p.Status == (int)CommonLib.TaskStatus.InProgress) &&
             p.IsActive != false &&
             p.Action != (int)DatabaseAction.Delete)
             .OrderByDescending(p => p.DueDate)
             .Take(11)
             .AsNoTracking()
             .ToListAsync();

             return tasks.Select(p => new QueriesDeshboardTask
    {
        Id = p.Id,
        ParentId = p.ParentId,
        Title = p.Title,
        ...
        Responsible = p.Responsible != null
            ? JsonConvert.DeserializeObject<QueriesPersonDto>(p.Responsible)
            : null,
        CoResponsible = p.CoResponsibles != null
            ? JsonConvert.DeserializeObject<List<QueriesGroupDto>>(p.CoResponsibles)
            : null,
        Status = p.Status,
        CreatedBy = p.CreatedBy != null
            ? JsonConvert.DeserializeObject<QueriesPersonDto>(p.CreatedBy)
            : null
    }).ToList();
        }
        ...
        private IList<EntityAction> GetUserAction(string createdby, string responsible, string coresponsible, bool hasSubTask, int? roleId, bool hasMeeting)
        {
            ...
            {
                if (roleId.HasValue && (
                    roleId.Value == (int)RolesType.User || 
                    roleId.Value == (int)RolesType.Guest))
                {
        ...
        private IList<EntityAction> GetUserSubTaskAction(string createdby, string responsible, int? roleId, string subTaskResponsible, string subTaskCreatedby, bool isMeetingTask)
        {
            ...
            else
            {
                if (roleId.HasValue && roleId.Value == (int)RolesType.Transient)
                {
                    ...
                    return _configuration.GetSection("SubTaskFullPermissions:Actions").Get<List<EntityAction>>();
                else if (roleId.HasValue && (roleId.Value == (int)RolesType.Guest || roleId.Value == (int)RolesType.User))
                    return _configuration.GetSection("SubTaskLeastPermissions:Actions").Get<List<EntityAction>>();
                else if (roleId.HasValue && (roleId.Value == (int)RolesType.AssistantDocumentMember))
                {
                    if (!isMeetingTask)
        ...
        private async Task<IList<EliteTask>> GetTaskByPaginateForDashBoard(
    Expression<Func<EliteTask, bool>> roles = null)
        {
            try
            {
                // Base query (common for all flows)
                IQueryable<EliteTask> query = this.GetDBSet<EliteTask>()
                    .Where(p =>
                        (p.Status == (int)CommonLib.TaskStatus.Assigned ||
                         p.Status == (int)CommonLib.TaskStatus.InProgress) &&
                        p.IsActive != false &&
                ...
                // Admin / Transient / Swagger (auth bypass)
                else
                {
                    // Apply UID filter ONLY when UID exists
                    if (!string.IsNullOrWhiteSpace(this._UID))
                    {
                        query = query.Where(p =>
                            (!string.IsNullOrEmpty(p.Responsible) &&
                             p.Responsible.Contains(this._UID)) ||
                            (!string.IsNullOrEmpty(p.CoResponsibles) &&
                             p.CoResponsibles.Contains(this._UID)));
                    }
                }

                // Projection
                query = query.Select(s => new EliteTask
                {
                    Id = s.Id,
                    Title = s.Title,
                    Description = s.Description,
                ...
                query = OrderBy.ApplyOrderBy(
                    query,
                    new OrderByExpression<EliteTask, DateTime?>(p => p.DueDate, false),
                    new OrderByExpression<EliteTask, string>(p => p.Title, false));
            ...
            {
                Console.WriteLine($"[ERROR] GetTaskByPaginateForDashBoard failed. " +
                    $"RolesFilterPresent={roles != null}, " +
                    $"UID={this._UID ?? "NULL"}");
                    Console.WriteLine(ex);
                    return new List<EliteTask>();
            }
        }
    ...
    int? page, int listType, string searchKeywords, HttpResponse response, FilterActionEnum taskFilterAction, string uid)
        {
            ITaskRolesAndRightFilterBuilder<EliteTask> rolesBuilder = TaskRolesAndRightsFilterFactory.GetTaskRolesAndRightsFilterObject(taskFilterAction, this.rolesPermissions.UserRolesAndRights, this._requestContext);
