...
namespace Elite.Task.Microservice.Application.CQRS.ExternalService
{
    public class UserService : IUserService
    {
        ...
        public async Task<List<CommitteeManager>> GetCommitteeManagersAsync(long committeeId)
        {
            this._httpHelper = new HttpClientHelper(_configuration.GetSection("AdminService:BaseUrl").Value, false);
            this._httpHelper.SetRequestHeaderForSecureID(_context);
            var response = await _httpHelper.HttpClient.GetAsync(new Uri(_httpHelper.HttpClient.BaseAddress.ToString()) + _configuration.GetSection("AdminService:ApiLink:GetCMInCommittee").Value + committeeId);
            if ((int)response.StatusCode == (int)System.Net.HttpStatusCode.OK)
            {
                var committeeManagers = JsonConvert.DeserializeObject<List<CommitteeManager>>(await response.Content.ReadAsStringAsync());
                if (committeeManagers.Count > 0)
                    return committeeManagers;
            }
            return new List<CommitteeManager>();
        }
        ...
        public async Task<CommitteeConfigSettings> GetCommitteeConfigSettingsAsync(long committeeID)
        {

            this._httpHelper = new HttpClientHelper(_configuration.GetSection("UserService:BaseUrl").Value, false);
            this._httpHelper.SetRequestHeaderForSecureID(_context);
            var topicGetResponse = await _httpHelper.HttpClient.GetAsync(new Uri(_httpHelper.HttpClient.BaseAddress.ToString()) + _configuration.GetSection("UserService:ApiLink:GetCommitteeConfigSettings").Value + committeeID);
            if ((int)topicGetResponse.StatusCode == (int)System.Net.HttpStatusCode.OK)
            {
                var commconfigsettings = JsonConvert.DeserializeObject<CommitteeConfigSettings>(await topicGetResponse.Content.ReadAsStringAsync());
                if (commconfigsettings.Id > 0)
                    return commconfigsettings;
            }
            return new CommitteeConfigSettings();
        }
