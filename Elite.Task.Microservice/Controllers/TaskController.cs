...
namespace Elite_Task.Microservice.Controllers
{
    ...
    public class TaskController : ControllerBase
    {
        ...
        #endregion
        #region Change FollowUp status  
        [HttpPost("follow-up")]
        [ProducesResponseType((int)HttpStatusCode.OK)]
        [ProducesResponseType((int)HttpStatusCode.BadRequest)]
        public async Task<IActionResult> FollowUpFlag([FromBody] FollowUpUpdateDto dto)
        {
            try
            {
                var updated = _taskRepo.UpdateFollowUpStatus(dto.TaskId, dto.FollowUp);
                if (updated)
                {
                    return Ok();
                }
