...
namespace Elite_Task.Microservice
{
    public class Startup
    {
        ...
        // Commented this function to check only Azure DB Autherntication with OIDC
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddEntityFrameworkNpgsql()
                ...
                {
                    opt.UseNpgsql(
                        Configuration.GetConnectionString("eliteTaskConnectionString"),
                        sqlOption =>
                        {
                            sqlOption.MigrationsAssembly(typeof(Startup).Assembly.GetName().Name);
                ...
                .AddDbContext<EliteLoggerContext>(options =>
                    options.UseNpgsql(
                        Configuration.GetConnectionString("eliteLogDbConnection")));
            ...
            /*if (Convert.ToBoolean(Configuration.GetSection("UseOIDC").Value))
            {
                ...
                services.AddTransient<Func<IConfiguration, IAuthTokenServices>>(
                    sp => (IConfiguration c) => new AuthTokenServices(c));
        ...
        public void Configure(IApplicationBuilder app, IHostingEnvironment env, ILoggerFactory loggerFactory)
        {
            ...
            if (env.IsDevelopment())
            {
                ...
                loggerFactory.AddLog4Net(
                    Configuration.GetSection("Logging:Dev_Log4NetConfigFile:Name").Value);
            ...
            else
            {
                ...
                {
                    loggerFactory.AddLog4Net(
                        Configuration.GetSection("Logging:Prod_Log4NetConfigFile:Name").Value);
                }
                if (env.IsStaging())
                {
                    loggerFactory.AddLog4Net(
                        Configuration.GetSection("Logging:Staging_Log4NetConfigFile:Name").Value);
                }
            }
