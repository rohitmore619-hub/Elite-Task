...
namespace Elite.Common.Utilities.Attachment.Delete.MapOrphans
{
    public class AttachmentService : IAttachmentService
    {
        ...
        public AttachmentService(IConfiguration configuration)
        {
            ...
            this._httpHelper = new HttpClientHelper.HttpClientHelper(_configuration.GetSection("AttachmentService:BaseUrl").Value, false);
        }
        ...
        public async Task<byte[]> getAttachmentByteCode(string guid)
        {
            var baseUrl = _httpHelper.HttpClient.BaseAddress.ToString();
            var apiPath = _configuration.GetSection("AttachmentService:ApiLink:GetAttachmentById").Value;
            var fullUrl = new Uri(baseUrl + apiPath + guid);
            
            var response = await _httpHelper.HttpClient.GetAsync(fullUrl);
